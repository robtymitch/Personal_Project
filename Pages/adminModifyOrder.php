...
$pdo=MySQLDB::connect();
$query=$pdo->prepare('SELECT Type, Description FROM maintenance_requests WHERE Request_Id=?');
$query->execute([$_GET['id']]);
$row = $query->fetch();

if(count($_POST)>0){
	$error=Admin::adminUpdateOrder($_POST);
	if(isset($error{0})){
		$message=$error;
		$alert_type='danger';
	} else {
        $message='The request has been updated please return to work orders to view it <a href="adminPage.php">HERE</a>';
		$alert_type='success';
    }
}
